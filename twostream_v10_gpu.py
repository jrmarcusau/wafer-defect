...
Optimized for Quadro RTX 5000 (16 GB VRAM):
- Mixed Precision (AMP)
- cudnn.benchmark + TF32
- Larger batch size (32) as safe starting point
- DataLoader: pin_memory + persistent_workers
- non_blocking=True on GPU transfers
"""
...
IMG_SIZE        = 256
EDGE_SIZE       = 512
BATCH_SIZE      = 32           # good starting point for 16 GB with AMP
Z_DIM           = 256
NUM_CLUSTERS    = 20
WARMUP_EPOCHS   = 20
TOTAL_EPOCHS    = 100
...
class WaferDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Polar branch
        polar = cv2.warpPolar(
            arr, (self.img_size, self.img_size),
        ...
        hog_polar = hog(polar, orientations=HOG_ORI, pixels_per_cell=HOG_PPC,
        ...
        coord = torch.cat([xs, ys], dim=0)
        x_polar = torch.cat([polar_t, edgep_t, coord], dim=0)

        # Cartesian branch
        cart = cv2.resize(arr, (self.img_size, self.img_size), interpolation=cv2.INTER_LINEAR)
        edge_cart256 = build_cart_edge_256(arr)
        hog_cart = hog(cart, orientations=HOG_ORI, pixels_per_cell=HOG_PPC,
                       cells_per_block=HOG_CPB, feature_vector=True).astype(np.float32)
        ...
        x_cart = torch.cat([cart_t, edgec_t, coord], dim=0)
...
# ────────────────────────────────────────────────────
#   Training loop (AMP + tuned DataLoader)
# ────────────────────────────────────────────────────
def train():
    ...
    ae_polar = ConvAE(Z_DIM, IMG_SIZE).to(DEVICE)
    ae_cart  = ConvAE(Z_DIM, IMG_SIZE).to(DEVICE)
    mse_fn = nn.MSELoss()
    ...
    for epoch in range(1, TOTAL_EPOCHS+1):
        ...
        if epoch > WARMUP_EPOCHS and epoch % UPDATE_INTERVAL == 1:
            ...
            with torch.no_grad():
                ...
                for x_pol, x_car, hog_pol, hog_car, line_vec, idxs in eval_loader:
                    x_pol = x_pol.to(DEVICE, non_blocking=True)
                    x_car = x_car.to(DEVICE, non_blocking=True)
        ...
        if epoch % UPDATE_INTERVAL == 0 or epoch == TOTAL_EPOCHS:
            ...
            save_checkpoint({
                ...
                'cluster_centers': centers.detach().cpu() if centers is not None else None
            }, epoch)

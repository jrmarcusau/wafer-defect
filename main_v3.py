...
----------------------------
✓ IMG_SIDE       : 1024  (keeps thin scratches, full rings)  
✓ Augmentations  : Resize → CenterCrop (no random crop) + flips + 90° rotation  
✓ tqdm progress  : epoch-level bar

Outputs
-------
outputs/v1/
    type_1/ … type_N/      full wafer images
...
IMG_SIDE   = 512              # ← request
BATCH      = 16                # adjust if GPU RAM limited
EPOCHS     = 30
MAX_TYPES  = 10
...
class WaferDS(Dataset):
    def __init__(self, root: Path):
        ...
        if not self.files:
            raise RuntimeError("No wafer images found in", root)
...
def train_ssl(ds: WaferDS):
    ...
    loader = DataLoader(ds,
                        ...
                        shuffle=True,
                        drop_last=True,
                        num_workers=0)      # 0 workers = safest on Windows

    for epoch in range(1, EPOCHS + 1):
        net.train(); running = 0.0

        # progress bar over the batches in this epoch (no live loss)
        for v1, v2, _ in tqdm(loader,
        ...
        print(f"[{epoch:02}/{EPOCHS}] epoch-avg loss {running / len(loader):.4f}")

    return net

# ----------------------------------------------------------------- clustering
def cluster(net: Encoder, ds: WaferDS):
...
def main():
    ds = WaferDS(DATA_DIR)
    print(f"Found {len(ds)} wafers.")
    enc = train_ssl(ds)
    labels, names, k = cluster(enc, ds)
    save(labels, names, k)
    print(f"✅ Done – {k} clusters written to {OUT_DIR}")

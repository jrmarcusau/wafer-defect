...
semisup_pipeline.py
────────────────────
End-to-end wafer-clustering pipeline in **three clean stages**:

1.  **Duplicate removal**  
    • Drops files in *Unlabeled/** whose basename already exists in any
      *Label_*/ folder (in-memory: originals stay on disk).
...
2.  **Noise filter**  
    ...
    • Classifies the (deduplicated) *Unlabeled/** set; wafers with
      **p(noise) ≥ 0.90** are discarded from clustering.

3.  **Semi-supervised dual-stream DEC**  
    • Polar + Cartesian streams; input channels = gray + Sobel + Coord.  
    • 30 real clusters + 4 Weak buckets (ids 30-33).  
    • Loss = reconstruction + 0.1·KL(DEC) + 0.3·CE(seeds) + 0.05·prototype.  
    • Early DEC (warm-up 2 epochs) and centroid refresh every 5 epochs.  
    • τ = 0.65 confidence gate for “strong’’ assignments.  # ← lowered from 0.70

Outputs (`<out_dir>/`)  
    checkpoints/ figures/ summaries/{top9,random9}/ type_<id>/  
    (id = 0-29 or 30-33 for Weak-0…3)

Run
----
python semisup_pipeline.py \
       --data_dir  data \
       --out_dir   outputs/semisup_all
"""
...
def train_dataset(root: str, out_dir: str) -> None:  # noqa: C901 (long but clear)
    ...
    for epoch in range(1, TOTAL_EPOCHS + 1):
        ...
        if epoch % UPDATE_INT == 0 or epoch == TOTAL_EPOCHS:
            ...
            confidence = q_all.max(1).values.numpy()

            # strong vs weak
            strong_mask = confidence >= TAU_STRONG
            weak_bin = np.digitize(
                confidence, np.quantile(confidence[~strong_mask], [0.25, 0.5, 0.75])
            )
            final_label = cluster_id.copy()
            final_label[~strong_mask] = K_CLUSTERS + weak_bin[~strong_mask]
            ...
            if epoch % TSNE_INT == 0:
                save_tsne(out_dir, epoch, feats.numpy(), final_label)
            ...
            # copy buckets
            for pth, lbl in zip(path_list, final_label):
                dst_dir = os.path.join(out_dir, f"type_{lbl}")
                ensure_dirs(dst_dir)
                shutil.copy2(pth, os.path.join(dst_dir, os.path.basename(pth)))

            # final weak-bucket re-cluster (30–33 → type_weak_0…3)
            if epoch == TOTAL_EPOCHS:
                weak_idx = np.where(final_label >= K_CLUSTERS)[0]
                if len(weak_idx) > 0:
                    weak_feats = feats[weak_idx]
                    weak_km = KMeans(N_WEAK, n_init=10, random_state=SEED).fit(weak_feats)
                    for s in range(N_WEAK):
                        ensure_dirs(os.path.join(out_dir, f"type_weak_{s}"))
                    for i, sub_lbl in zip(weak_idx, weak_km.labels_):
                        src = path_list[i]
                        dst = os.path.join(out_dir, f"type_weak_{sub_lbl}", os.path.basename(src))
...
def main() -> None:
    ...
    parser.add_argument("--out_dir",  default='outputs/sup_v3')
